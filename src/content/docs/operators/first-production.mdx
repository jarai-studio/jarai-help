...
<VideoHelper src="first-production.mp4" title="Create your first production">
...
How much you steer a production depends on the account's **execution mode** (set in
[Account settings](/operators/account-settings/)):

- **Autonomous** — runs end-to-end without stopping; you only step in if a step **fails** (use
  the **retry** button) or it reaches a **gate**.
...
From the production detail page you can **retry** a failed step, act on a **PendingReview**
gate, and at the end **Approve to publish** or **Reject**. If the account has **client approval
required**, finished productions wait for the customer's sign-off before publishing.

<Aside type="tip">
For recovering failed or stuck productions — retries, dead-letters, provider outages — see
[Troubleshooting](/operators/troubleshooting/).
</Aside>
